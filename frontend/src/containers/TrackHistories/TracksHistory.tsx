...
const TracksHistory = () => {
  ...
  const trackHistory = useSelector(
    (state: RootState) => state.tracksHistory.trackHistory,
  );
  const loading = useSelector(
    (state: RootState) => state.tracksHistory.isLoading,
  );
      ...
      {loading ? (
        ...
        <>
          {trackHistory.map((trackFromHistory) => (
            <TrackHistoryCard
              key={trackFromHistory._id + 1}
              trackFromHistory={trackFromHistory}
            />
          ))}
        </>
      )}
